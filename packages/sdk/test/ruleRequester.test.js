...
const { ruleRequester } = await import("sdk/src/ruleRequester.js");
...
describe("ruleRequester", () => {
  ...
  it("should be called with fallback to default values for Domain and Path", async () => {
    ...
    expect(makeRequestMock).toBeCalledWith(
      "https://assets.adobetarget.com/test-org-id/production/v1/test-datastream/rules.json",
      expectedOptions,
    );
  });
  ...
  it("should be called with custom values for Domain", () => {
    ...
    expect(makeRequestMock).toBeCalledWith(
      "https://custom-domain/test-org-id/production/v1/test-datastream/rules.json",
      expectedOptions,
    );
  });
  it("should be called without domain - we have paths without Domain in Akamai", () => {
    ...
    ruleRequester(customClientOptions);
    expect(makeRequestMock).toBeCalledWith(
      "/test-org-id/production/v1/test-datastream/rules.json",
      expectedOptions,
    );
  });
